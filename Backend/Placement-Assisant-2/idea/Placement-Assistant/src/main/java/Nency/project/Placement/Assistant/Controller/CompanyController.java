...
public class CompanyController {
    ...
    @GetMapping("/companiesForStudent/{email}")
    public ResponseEntity<?> getCompaniesForStudent(@PathVariable String email) {
        ...
        if (batch == null || batch.trim().isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Batch not set for the student");
        }
        // Log the batch for debugging:
        System.out.println("Fetching company names for batch: " + batch);
        List<Company> companies = companyRepository.findByBatch(batch);
        if (companies.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("No companies found for batch: " + batch);
        }
